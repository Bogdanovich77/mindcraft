...
export class TheoryOfMindEngine {
    ...
    async initialize(personality) {
        try {
            ...
            await this.mentalModelManager.initialize(personality);
            await this.intentionPredictor.initialize(personality);
            await this.emotionalIntelligence.initialize(personality);
            await this.socialReasoning.initialize(personality);
    ...
    async processObservation(targetAgentId, observation, context, observerPersonality) {
        ...
        try {
            ...
            const socialContext = this.socialReasoning.analyzeSocialContext(socialReasoningInput);
    ...
    async reset() {
        ...
        try {
            await this.mentalModelManager.reset();
            await this.intentionPredictor.reset();
            await this.emotionalIntelligence.reset();
            await this.socialReasoning.reset();
            this.metrics = this.initializeMetrics();
    ...
    async cleanup() {
        ...
        try {
            // Note: MentalModelManager doesn't have cleanup method
            // Use reset instead
            this.mentalModelManager.reset();
            // Note: IntentionPredictor doesn't have cleanup method
            // Use reset instead
            this.intentionPredictor.reset();
            // Note: EmotionalIntelligence doesn't have cleanup method
            // Use reset instead
            this.emotionalIntelligence.reset();
            // Note: SocialReasoning doesn't have cleanup method
            // Use reset instead
