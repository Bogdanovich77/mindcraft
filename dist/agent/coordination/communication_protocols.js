 ...
 */
import { MessageType, MessagePriority, MessageDeliveryStatus, ChannelState, BroadcastAudience, BroadcastFrequency, BroadcastStatus, CoordinationEventType } from './coordination_types.js';
/**
...
export class CommunicationProtocols {
    ...
    async sendToChannel(message) {
        ...
        const deliveryPromises = channel.participants.map(participantId => this.deliverToAgent(participantId, message));
    ...
    async sendMulticastMessage(message) {
        ...
        try {
            const deliveryPromises = message.recipients.map(recipientId => this.deliverToAgent(recipientId, message));
            const results = await Promise.allSettled(deliveryPromises);
            const failures = results.filter(result => result.status === 'rejected');
            if (failures.length > 0) {
     ...
     */
    checkChannelPermissions(channel, message) {
    ...
    checkSpamProtection(message) {
        ...
        if (spamProtection.duplicateDetection) {
            ...
            const isDuplicate = recentMessages.some(sent => sent.payload.content === message.payload.content &&
        ...
        if (spamProtection.blacklist.length > 0) {
            ...
            const isBlacklisted = spamProtection.blacklist.some(term => content.includes(term.toLowerCase()));
            if (isBlacklisted)
                return false;
        }
        return true;
     ...
     * Update performance metrics
     */
    updatePerformanceMetrics(messagesProcessed, processingTime) {
     ...
     */
    sendMessage(type, payload, priority = MessagePriority.MEDIUM, options = {}) {
     ...
     */
    sendBroadcast(type, content, audience = BroadcastAudience.ALL, frequency = BroadcastFrequency.ONCE) {
     ...
     */
    determineBroadcastRecipients(audience) {
     ...
     */
    createChannel(name, type, participants, options = {}) {
