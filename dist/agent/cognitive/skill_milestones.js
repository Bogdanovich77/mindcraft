 ...
 */
import { SkillType, MilestoneType, MilestoneTier } from './skill_types.js';
export class SkillMilestoneSystem {
    config;
    milestones = new Map();
    specializations = new Map();
    milestoneProgress = new Map();
    specializationProgress = new Map();
    activeSpecializations = new Set();
    constructor(config) {
        this.config = {
     ...
     */
    addMilestones(skillType, milestones) {
     ...
     */
    addSpecializations(skillType, specializations) {
     ...
     */
    updateMilestoneProgress(skillType, skillLevel, experience, practiceData, successRate = 0) {
     ...
     */
    updateSpecializationProgress(skillType, skillLevel, practiceData, completedMilestones) {
     ...
     */
    unlockSpecialization(specializationId) {
     ...
     */
    getActiveSpecializations(skillType) {
        const specializations = this.specializations.get(skillType) || [];
        return specializations.filter(spec => this.activeSpecializations.has(spec.id) && spec.baseSkill === skillType);
    }
    /**
     * Get available milestones for a skill
     */
    getAvailableMilestones(skillType) {
     ...
     */
    getAvailableSpecializations(skillType) {
        const specializations = this.specializations.get(skillType) || [];
     ...
     */
    handleMilestoneAchievement(milestone) {
        // Apply rewards
     ...
     */
    calculateMilestoneBonuses(skillType) {
        const milestones = this.milestones.get(skillType) || [];
        const achievedMilestones = milestones.filter(m => this.milestoneProgress.get(m.id)?.isAchieved);
        let skillBonus = 1.0;
                ...
                switch (reward.type) {
                    case 'bonus':
                        if (typeof reward.value === 'number') {
                            if (reward.name.includes('skill'))
                                skillBonus *= reward.value;
                            if (reward.name.includes('learning'))
                                learningBonus *= reward.value;
                            if (reward.name.includes('experience'))
                                experienceBonus *= reward.value;
                        }
                        break;
                    case 'ability':
     ...
     */
    calculateSpecializationBonuses(skillType) {
...
//# sourceMappingURL=skill_milestones.js.map