...
export class GoalSystem {
     ...
     */
    generateSocialGoals(agentState) {
        if (!this.socialState) {
            return [];
        }
        const socialGoals = [];
     ...
     */
    applySocialInfluenceToGoals(goals, agentState) {
     ...
     */
    createCollaborativeGoal(goalName, description, collaborators, agentState) {
        const now = Date.now();
        return {
            ...
            ethicalScore: 0.8,
            expectedLearning: [
                { type: 'skill', area: 'teamwork', expectedGain: 0.7 },
                { type: 'skill', area: 'communication', expectedGain: 0.6 },
                { type: 'skill', area: 'coordination', expectedGain: 0.5 }
            ],
     ...
     */
    calculateCollaborativePriority(collaborators, agentState) {
     ...
     */
    calculateSocialAlignment(collaborators) {
        if (!this.socialState || collaborators.length === 0) {
     ...
     */
    estimateCollaborativeTime(description, collaboratorCount) {
     ...
     */
    async generateAntiIdleGoalsIfNeeded(agentState) {
        const activeGoals = [
     ...
     */
    async createPlanFromGoal(goalId, agentState) {
    ...
    async updateGoalFromPlanProgress(planId, progress, agentState) {
        ...
        if (progress.status) {
            switch (progress.status) {
                case 'completed':
                    associatedGoal.status = 'completed';
                    break;
                case 'failed':
                    associatedGoal.status = 'failed';
                    break;
                case 'active':
                    associatedGoal.status = 'active';
                    break;
                case 'paused':
                    associatedGoal.status = 'paused';
                    break;
            }
        }
     ...
     */
    mapGoalPriorityToPlanPriority(goalPriority) {
        if (goalPriority <= 1)
            return 'CRITICAL';
        if (goalPriority <= 3)
            return 'HIGH';
     ...
     */
    convertGoalRequirementsToPlanRequirements(goalRequirements) {
        if (!goalRequirements)
            return {};
...
//# sourceMappingURL=goal_system.js.map