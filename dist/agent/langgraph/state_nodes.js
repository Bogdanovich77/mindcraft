 ...
 */
export async function executionNode(state, bot, agent) {
 ...
 */
async function executeChatResponse(response, bot) {
    try {
        if (!bot || !bot.chat) {
 ...
 */
async function executeAction(action, state, bot, agent) {
    try {
 ...
 */
async function handleChatAction(action, state, bot) {
    try {
 ...
 */
async function handleMovementAction(action, state, bot) {
    try {
 ...
 */
async function handleDiggingAction(action, state, bot) {
    try {
 ...
 */
async function handleBuildingAction(action, state, bot) {
    try {
        ...
        const placeableItem = state.worldContext?.inventory?.items?.find((item) => item.name.includes('dirt') || item.name.includes('stone') || item.name.includes('wood'));
        if (!placeableItem) {
            return {
                success: false,
 ...
 */
async function handleCraftingAction(action, state, bot) {
    try {
 ...
 */
async function handleEatingAction(action, state, bot) {
    try {
        ...
        const foodItem = state.worldContext?.inventory?.items?.find((item) => item.name.includes('bread') || item.name.includes('apple') || item.name.includes('meat') ||
 ...
 */
async function handleHealingAction(action, state, bot) {
    try {
 ...
 */
async function handleSleepingAction(action, state, bot) {
    try {
 ...
 */
async function handleAttackAction(action, state, bot) {
    try {
        ...
        const hostileEntities = state.worldContext?.nearbyEntities?.filter((e) => e.hostile) || [];
        ...
        const entity = bot.nearestEntity((entity) => entity.id === target.id);
        if (entity) {
 ...
 */
async function handleDefenseAction(action, state, bot) {
    try {
        ...
        const hostileEntities = state.worldContext?.nearbyEntities?.filter((e) => e.hostile && (e.distance || 0) < 8) || [];
        if (hostileEntities.length > 0) {
 ...
 */
async function handleFleeAction(action, state, bot) {
    try {
        ...
        const hostileEntities = state.worldContext?.nearbyEntities?.filter((e) => e.hostile && (e.distance || 0) < 16) || [];
        if (hostileEntities.length === 0) {
            return {
                success: false,
 ...
 */
async function handleExplorationAction(action, state, bot) {
    try {
 ...
 */
async function handlePatrolAction(action, state, bot) {
    try {
 ...
 */
async function handleOrganizeAction(action, state, bot) {
    try {
 ...
 */
async function handleAcceptHelpAction(action, state, bot) {
    try {
 ...
 */
async function handleRequestHelpAction(action, state, bot) {
    try {
 ...
 */
async function handleWaitAction(action, state, bot) {
    try {
 ...
 */
async function handleUnknownAction(action, state, bot) {
    try {
 ...
 */
async function updateMandateFromConversation(state, bot) {
    try {
 ...
 */
function shouldClearMandate(state, mandate) {
    try {
 ...
 */
function validateAndCleanChatMessage(message) {
    try {
...
//# sourceMappingURL=state_nodes.js.map