...
export class SemanticMemory {
     ...
     */
    async storeRelationship(relationship, fromConcept) {
     ...
     */
    async updateThreatKnowledge(pattern) {
     ...
     */
    rankByRelevance(concepts, query) {
     ...
     */
    updateRelationshipIndex(concept) {
    ...
    async cleanupLeastImportantConcepts() {
        if (this.concepts.size <= this.maxConcepts * 0.8)
            return;
    ...
    calculateRelevanceScore(concept, query) {
        const queryLower = query.query.toLowerCase();
        let score = 0;
        // Name matching
        if (concept.name.toLowerCase().includes(queryLower)) {
...
//# sourceMappingURL=semantic_memory.js.map