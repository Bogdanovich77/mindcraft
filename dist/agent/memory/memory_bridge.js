...
export class MemoryBridge {
     ...
     */
    async migrateLegacyMemory(legacyMemoryBank, memorySystem) {
        ...
        try {
            ...
            if (legacyMemoryBank.memory && typeof legacyMemoryBank.memory === 'object') {
                ...
                for (const [name, location] of Object.entries(locationData)) {
                    try {
                        await this.migrateLocation(name, location, memorySystem);
                        migratedLocations++;
                    }
                    catch (error) {
            ...
            if (legacyMemoryBank.locations) {
                for (const [name, data] of Object.entries(legacyMemoryBank.locations)) {
                    try {
                        await this.migrateExtendedLocationData(name, data, memorySystem);
                        migratedConcepts++;
                    }
                    catch (error) {
     ...
     */
    createLegacyWrapper(memorySystem) {
     ...
     */
    async exportToLegacyFormat(memorySystem) {
        const legacyFormat = {
     ...
     */
    async migrateLocation(name, location, memorySystem) {
    ...
    async migrateExtendedLocationData(name, data, memorySystem) {
    ...
    async createLocationConcepts(locationData, memorySystem) {
        ...
        for (const type of Array.from(locationTypes)) {
            const typeConcept = {
                ...
                attributes: {
                    category: 'location',
                    locationType: type,
                    count: Array.from(Object.values(locationData)).filter((loc) => loc.type === type).length
                },
                relationships: [],
                lastAccessed: Date.now(),
                importance: 0.6
