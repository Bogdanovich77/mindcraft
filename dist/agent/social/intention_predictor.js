 ...
 */
import { IntentionType, EvidenceType } from './tom_types.js';
/**
...
export class IntentionPredictor {
     ...
     */
    matchPattern(pattern, actions, context) {
        let matchScore = 0;
     ...
     */
    calculateSequenceMatch(patternSequence, actions) {
     ...
     */
    calculateContextMatch(requirements, context) {
     ...
     */
    contextHasRequirement(context, requirement) {
     ...
     */
    calculatePersonalityMatch(patternCorrelation, personality) {
     ...
     */
    createPredictionFromPattern(patternMatch, context) {
     ...
     */
    inferRequiredResources(intentionType) {
        const resourceMap = {
     ...
     */
    calculateIntentionProbability(pattern, context) {
     ...
     */
    calculateIntentionPriority(pattern, context) {
     ...
     */
    generateAlternativeIntentions(pattern, context) {
        const alternatives = [];
     ...
     */
    getSimilarIntentionTypes(intentionType) {
        const similarityMap = {
     ...
     */
    findPatternByType(intentionType) {
        for (const pattern of this.patterns.values()) {
     ...
     */
    createStorageIntention(context) {
        return {
     ...
     */
    generateTemporalPredictions(context) {
        const predictions = [];
     ...
     */
    createContinuationPrediction(recent, context) {
     ...
     */
    storePredictionHistory(agentId, predictions) {
     ...
     * Update confidence metrics
     */
    updateConfidenceMetrics(predictions) {
     ...
     */
    validatePrediction(agentId, predictionId, actualIntention) {
        const history = this.predictionHistory.get(agentId) || [];
     ...
     */
    detectIntentionChanges(agentId, currentPredictions, previousPredictions) {
        const changes = [];
        // Find new intentions
...
//# sourceMappingURL=intention_predictor.js.map