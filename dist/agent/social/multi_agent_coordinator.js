...
(function (MessagePriority) {
    ...
    MessagePriority[MessagePriority["BACKGROUND"] = 4] = "BACKGROUND";
})(MessagePriority || (MessagePriority = {}));
/**
 * Status of coordination activities
 */
export var CoordinationStatus;
(function (CoordinationStatus) {
    ...
    CoordinationStatus["TIMEOUT"] = "timeout";
...
export class MultiAgentCoordinator {
    ...
    bandwidthUsage;
    lastBandwidthReset;
    constructor(agentId, config = {}, getRelationship, getMentalState, predictIntention) {
        this.getRelationship = getRelationship;
        this.getMentalState = getMentalState;
        this.predictIntention = predictIntention;
     ...
     */
    async sendMessage(targetId, type, content, priority = MessagePriority.MEDIUM, requiresResponse = false, responseTo) {
     ...
     */
    async broadcastMessage(targetIds, type, content, priority = MessagePriority.MEDIUM) {
        const startTime = Date.now();
        try {
     ...
     */
    async formCollaborativeGoal(type, title, description, priority = MessagePriority.MEDIUM, requiredAgents = 2, requirements) {
        const startTime = Date.now();
        try {
            ...
            const invitationPromises = invitations.map(agentId => this.sendCollaborationInvitation(collaboration, agentId));
            const invitationResults = await Promise.allSettled(invitationPromises);
            const successfulInvitations = invitationResults.filter(result => result.status === 'fulfilled' && result.value).length;
     ...
     */
    async delegateTask(delegateeId, taskId, title, description, priority = MessagePriority.MEDIUM, requirements, deadline) {
        const startTime = Date.now();
        try {
     ...
     */
    async mediateConflict(conflictId, parties, proposedTerms) {
        const startTime = Date.now();
        try {
            ...
            const acceptances = mediationResults.filter(result => result.status === 'fulfilled' && result.value).length;
     ...
     * Check bandwidth availability
     */
    checkBandwidthAvailability(requiredBandwidth = 1) {
        const now = Date.now();
     ...
     */
    calculateUrgency(type, priority) {
        const baseUrgency = {
     ...
     */
    calculateImportance(type, priority) {
     ...
     */
    updateMessageMetrics(message, delivered, processingTime) {
     ...
     */
    updateBroadcastMetrics(messages, successfulDeliveries, totalTime) {
     ...
     */
    async identifyPotentialCollaborators(collaboration) {
     ...
     */
    async sendCollaborationInvitation(collaboration, agentId) {
     ...
     */
    updateCollaborationMetrics(collaboration, invitations, totalTime) {
     ...
     * Check delegatee capacity
     */
    async checkDelegateeCapacity(delegateeId, requirements) {
     ...
     */
    async sendTaskDelegationRequest(delegation) {
     ...
     */
    updateDelegationMetrics(delegation, accepted, totalTime) {
     ...
     */
    async initiateNegotiation(conflict) {
        const negotiation = {
     ...
     */
    async checkMediationEligibility(parties) {
     ...
     */
    async sendMediationRequest(mediation, agentId, terms) {
     ...
     */
    updateMediationMetrics(mediation, successful, totalTime) {
...
export class MultiAgentCoordinatorFactory {
     ...
     */
    static createDefault(agentId, getRelationship, getMentalState, predictIntention) {
     ...
     */
    static createHighPerformance(agentId, getRelationship, getMentalState, predictIntention) {
        const config = {
     ...
     */
    static createResourceConstrained(agentId, getRelationship, getMentalState, predictIntention) {
        const config = {
...
//# sourceMappingURL=multi_agent_coordinator.js.map