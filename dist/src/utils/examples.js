...
export class Examples {
    ...
    async load(examples) {
        ...
        if (!this.model)
            return; // Early return if no embedding model
    ...
    async getRelevant(turns) {
        ...
        if (this.model !== null) {
            ...
            this.examples.sort((a, b) => cosineSimilarity(embedding, this.embeddings[this.turnsToText(b)]) -
                cosineSimilarity(embedding, this.embeddings[this.turnsToText(a)]));
        }
        else {
            this.examples.sort((a, b) => wordOverlapScore(turn_text, this.turnsToText(b)) -
