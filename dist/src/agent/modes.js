...
async function say(agent, message) {
    ...
    if (agent.shut_up || !settings.narrate_behavior)
        return;
    ...
    },
    {
        ...
        update: function (agent) {
            if (world.shouldPlaceTorch(agent.bot)) {
                if (Date.now() - this.last_place < this.cooldown * 1000)
                    return;
                execute(this, agent, async () => {
...
async function execute(mode, agent, func, timeout = -1) {
    ...
    mode.active = true;
    // Wrap the function to handle PathStopped errors
    const wrappedFunc = async () => {
        try {
            ...
            if (error.message && error.message.includes('PathStopped')) {
                console.log(`[MODES] Mode ${mode.name} gracefully interrupted (PathStopped)`);
                // Clean up pathfinder state
                if (agent.bot.pathfinder) {
                    agent.bot.pathfinder.stop();
                }
                // Re-throw as a special interrupt signal that the action manager can handle
    ...
    let code_return = await agent.actions.runAction(`mode:${mode.name}`, wrappedFunc, { timeout });
    mode.active = false;
    // Check if the execution was interrupted by PathStopped
    if (code_return.message && code_return.message.includes('PathStopped')) {
        console.log(`[MODES] Mode ${mode.name} completed with expected pathfinder interruption`);
    ...
    let should_reprompt = interrupted_action && // it interrupted a previous action
