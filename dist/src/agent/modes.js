...
class ModeController {
    ...
    */
    ...
    async update() {
        ...
        const emergencyModes = modes_list.filter(mode => mode.interrupts && mode.interrupts.includes('all'));
        const normalModes = modes_list.filter(mode => !mode.interrupts || !mode.interrupts.includes('all'));
        // Check emergency modes first
        for (let mode of emergencyModes) {
            if (mode.on && !mode.paused && !mode.active) {
                await mode.update(_agent);
                if (mode.active)
                    break; // Exit early if emergency mode is active
            }
        }
