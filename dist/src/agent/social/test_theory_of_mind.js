...
class TheoryOfMindTestSuite {
    ...
    async runMentalModelTests() {
        ...
        await this.test('Mental state creation', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_1', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Mental state updates', async () => {
            ...
            await this.theoryOfMindEngine.processObservation(agentId, initialObservation, context, TEST_PERSONALITY);
            ...
            await this.theoryOfMindEngine.processObservation(agentId, updatedObservation, context, TEST_PERSONALITY);
        ...
        await this.test('Belief revision', async () => {
            ...
            await this.theoryOfMindEngine.processObservation(agentId, beliefObservation, context, TEST_PERSONALITY);
            ...
            await this.theoryOfMindEngine.processObservation(agentId, contradictoryObservation, updatedContext, TEST_PERSONALITY);
            ...
            const hasRevisedBelief = finalState.knowledge.epistemic.some(belief => belief.confidence < 0.8 // Lower confidence after contradiction
            );
            if (!hasRevisedBelief) {
    ...
    async runIntentionPredictorTests() {
        ...
        await this.test('Goal inference from actions', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_4', observation, context, TEST_PERSONALITY);
            if (!mentalState || !mentalState.intentions) {
                throw new Error('Intentions were not predicted');
            }
            const hasWoodGatheringIntention = mentalState.intentions.current.some(intention => intention.type.includes('gather') || intention.type.includes('wood') || intention.type.includes('resource'));
        ...
        await this.test('Plan recognition', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_5', observation, context, TEST_PERSONALITY);
            if (!mentalState || !mentalState.intentions) {
                throw new Error('Plan recognition failed');
            }
            const hasMiningPlan = mentalState.intentions.current.some(intention => intention.type.includes('mine') || intention.type.includes('explore') || intention.type.includes('resource'));
            if (!hasMiningPlan) {
        ...
        await this.test('Intention confidence scoring', async () => {
            ...
            const unclearState = await this.theoryOfMindEngine.processObservation(agentId, unclearObservation, context, TEST_PERSONALITY);
            ...
            const clearState = await this.theoryOfMindEngine.processObservation(agentId, clearObservation, context, TEST_PERSONALITY);
    ...
    async runEmotionalIntelligenceTests() {
        ...
        await this.test('Emotion recognition from behavioral cues', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_7', observation, context, TEST_PERSONALITY);
            if (!mentalState || !mentalState.emotions) {
                throw new Error('Emotions were not recognized');
            }
            const hasPositiveEmotion = Array.from(mentalState.emotions.current.keys()).some(emotion => ['happy', 'excited', 'joy', 'pleasure'].includes(emotion.toLowerCase()));
        ...
        await this.test('Empathy simulation', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_8', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Emotional state prediction', async () => {
            ...
            const prediction = await this.theoryOfMindEngine.predictBehavior(agentId, threatContext, 5000);
    ...
    async runSocialReasoningTests() {
        ...
        await this.test('Social context understanding', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_10', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Group dynamics prediction', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_11', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Social norm understanding', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('test_agent_12', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
    ...
    async runTheoryOfMindEngineTests() {
        ...
        await this.test('Behavior prediction', async () => {
            ...
            await this.theoryOfMindEngine.processObservation(agentId, observation, context, TEST_PERSONALITY);
            // Predict behavior
            const prediction = await this.theoryOfMindEngine.predictBehavior(agentId, context, 10000);
        ...
        await this.test('Perspective taking', async () => {
            ...
            await this.theoryOfMindEngine.processObservation(agentId, observation, context, TEST_PERSONALITY);
            // Take perspective
            const perspective = await this.theoryOfMindEngine.takePerspective(agentId, context, 3000);
        ...
        await this.test('Deception detection', async () => {
            ...
            const deception = await this.theoryOfMindEngine.detectDeception(agentId, communication, context);
            if (!deception) {
    ...
    async runIntegrationTests() {
        ...
        await this.test('Relationship manager integration', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation(agentId, observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Memory system integration', async () => {
            ...
            const hasSkillMemory = finalState.knowledge.procedural.some(skill => skill.confidence > 0.7);
            if (!hasSkillMemory) {
        ...
        await this.test('Personality system integration', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation(agentId, observation, context, aggressivePersonality);
            if (!mentalState) {
    ...
    async runPerformanceTests() {
        ...
        await this.test('Single observation processing time', async () => {
            ...
            const startTime = Date.now();
            await this.theoryOfMindEngine.processObservation('perf_test_agent', observation, context, TEST_PERSONALITY);
        ...
        await this.test('Concurrent agent processing', async () => {
            ...
            for (let i = 0; i < agentCount; i++) {
                ...
                promises.push(this.theoryOfMindEngine.processObservation(`concurrent_agent_${i}`, observation, context, TEST_PERSONALITY));
            }
        ...
        await this.test('Memory usage scaling', async () => {
            ...
            for (let i = 0; i < agentCount; i++) {
                ...
                await this.theoryOfMindEngine.processObservation(`memory_agent_${i}`, observation, context, TEST_PERSONALITY);
            }
        ...
        await this.test('Prediction performance', async () => {
            ...
            const startTime = Date.now();
            const prediction = await this.theoryOfMindEngine.predictBehavior(agentId, context, 5000);
    ...
    async runAdvancedFeatureTests() {
        ...
        await this.test('Cultural context modeling', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('cultural_agent', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
        ...
        await this.test('Group behavior prediction', async () => {
            ...
            const mentalState = await this.theoryOfMindEngine.processObservation('group_agent', observation, context, TEST_PERSONALITY);
            if (!mentalState) {
