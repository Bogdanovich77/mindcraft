import { GoalStatus, GoalPriority, DependencyType } from './goal_types.js';
/**
 * Priority calculation strategies
 */
export var PrioritizationStrategy;
(function (PrioritizationStrategy) {
    PrioritizationStrategy["WEIGHTED_SUM"] = "weighted_sum";
    PrioritizationStrategy["FUZZY_LOGIC"] = "fuzzy_logic";
    PrioritizationStrategy["UTILITY_BASED"] = "utility_based";
    PrioritizationStrategy["LEARNING_ADAPTIVE"] = "learning_adaptive";
    PrioritizationStrategy["CONTEXT_DYNAMIC"] = "context_dynamic";
    PrioritizationStrategy["MULTI_OBJECTIVE"] = "multi_objective"; // Pareto optimal multi-objective
...
export class GoalPrioritizationEngine {
     ...
     */
    async prioritizeGoals(goals, context, strategy) {
        const selectedStrategy = strategy || this.strategy;
        // Filter active goals
        const activeGoals = goals.filter(goal => goal.status === GoalStatus.PENDING ||
        ...
        for (const goal of activeGoals) {
            const priorityResult = await this.calculateGoalPriority(goal, context, selectedStrategy, contextFactors);
     ...
     */
    async calculateGoalPriority(goal, context, strategy, contextFactors) {
     ...
     * Calculate individual factor scores
     */
    async calculateFactorScores(goal, context, contextFactors) {
     ...
     */
    async calculateUrgency(goal, context, contextFactors) {
        const now = Date.now();
     ...
     */
    async calculateImportance(goal, context, contextFactors) {
     ...
     */
    async calculateFeasibility(goal, context, contextFactors) {
     ...
     */
    async calculateResource(goal, context, contextFactors) {
        // Resource efficiency
     ...
     */
    async calculateAlignment(goal, context, contextFactors) {
        // Personality fit
     ...
     */
    calculateUtilityBased(factors, goal, context) {
     ...
     */
    calculateContextDynamic(factors, contextFactors) {
     ...
     */
    calculateMultiObjective(factors, goal, context) {
    ...
    calculateOpportunityCost(goal, context) {
        ...
        ].filter((g) => g.id !== goal.id && g.status === GoalStatus.ACTIVE).length || 0;
        return Math.min(1, competingGoals * 0.1);
    }
    calculateValueDecay(goal, context) {
        // Calculate how quickly goal value decays over time
        if (!goal.deadline)
            return 0.1; // Low decay for non-deadline goals
    ...
    calculateStrategicValue(goal, context) {
        // Assess strategic value based on goal level and alignment
        if (goal.level === 'strategic')
            return 0.9;
        if (goal.level === 'tactical')
            return 0.7;
        if (goal.level === 'operational')
            return 0.5;
        return 0.5;
    }
    calculateImpactScope(goal, context) {
        // Assess scope of impact
        const tags = goal.tags || [];
        if (tags.includes('global') || tags.includes('empire'))
            return 1.0;
        if (tags.includes('community') || tags.includes('team'))
            return 0.7;
        if (tags.includes('personal'))
            return 0.5;
        return 0.5;
    }
    calculateResourceAvailability(goal, context) {
    ...
    adjustFactorsForContext(factors, contextFactors) {
    ...
    applyDependencyAdjustments(priorityScore, goal, context) {
    ...
    applyReactiveConsiderations(priorityScore, goal, context) {
    ...
    generateReasoning(factors, priorityScore, strategy) {
...
class AdaptiveWeightSystem {
    baseFactors;
    adaptationHistory = [];
    constructor(baseFactors) {
        this.baseFactors = { ...baseFactors };
    }
    getAdaptiveFactors(goal, context) {
    ...
    adjustWeightsForLowSuccess(factors) {
    ...
    updateBaseFactors(newFactors) {
        this.baseFactors = { ...newFactors };
    }
    recordAdaptation(success) {
...
class PriorityContextAnalyzer {
    analyzeContext(context, goals) {
        return {
            hasUrgentThreats: context.environmentalConditions.some(c => c.type === 'danger' && c.severity > 0.7),
    ...
    assessResourceScarcity(context) {
    ...
    assessTimePressure(goals) {
        // Check if there's high time pressure
        const urgentGoals = goals.filter(g => g.deadline && (g.deadline - Date.now()) < (60 * 60 * 1000) // 1 hour
        ).length;
        return urgentGoals > 2;
    }
    assessSocialDynamics(context) {
        return {
...
class PriorityLearningSystem {
    ...
    recordPrioritization(goals, context) {
        if (!this.enabled)
            return;
    ...
    recordOutcome(prioritizationId, outcome) {
        if (!this.enabled || prioritizationId >= this.learningHistory.length)
            return;
    ...
    extractPatterns(history) {
        ...
        return {
            avgUrgencyWeight: history.reduce((sum, h) => sum + h.goals[0]?.factors?.urgency || 0, 0) / history.length,
            avgImportanceWeight: history.reduce((sum, h) => sum + h.goals[0]?.factors?.importance || 0, 0) / history.length
        };
    }
}
