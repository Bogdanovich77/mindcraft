...
start "Node.js Agent Core" cmd /c "cd backend/node-core && node main.js --profiles "./profiles/SlaveOne.json" "./profiles/SlaveTwo.json" "./profiles/SlaveThree.json" "./profiles/Loner.json" "./profiles/MasterChief.json""
...
start "FastAPI Gateway" cmd /c "cd backend/fastapi-gateway && if not exist .venv (python -m venv .venv) && .venv\Scripts\activate && pip install -r requirements.txt && python main.py"
