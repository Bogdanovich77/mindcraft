...
// Test classes
class FrontendIntegrationTester extends EventEmitter {
  constructor() {
    super();
  ...
  async testDataFlow() {
    ...
    try {
      ...
      if (socketTest.connected) {
        ...
        const agentListTest = await this.testAgentListReception(socketTest.socket);
        recordTestResult('Agent List Reception', agentListTest.received, agentListTest.error);
        
        // Test agent state updates
        const agentUpdateTest = await this.testAgentStateUpdates(socketTest.socket);
  ...
  async testAgentListReception(socket) {
    return new Promise((resolve) => {
      const timeout = setTimeout(() => {
      ...
      socket.on('agents-status', (data) => {
        clearTimeout(timeout);
        const hasExpectedAgents = Array.isArray(data) || (data.agents && Array.isArray(data.agents));
        resolve({ received: true, error: hasExpectedAgents ? null : 'Invalid agent data format' });
      });
  ...
  async testAgentStateUpdates(socket) {
    return new Promise((resolve) => {
      const timeout = setTimeout(() => {
  ...
  async testDataTransformation() {
    try {
      ...
      const mockBackendData = [
        ...
        { name: 'Slave1', in_game: false },
        { name: 'AlphaSurvivor', in_game: true }
      ];
  ...
  async testPerformanceReliability() {
    ...
    try {
      ...
      const raceConditionTest = await this.testRaceConditionFix();
  ...
  async testRaceConditionFix() {
    try {
      ...
      return new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => {
          resolve({ fixed: responseCount === expectedResponses, error: null });
        }, 5000);
        ...
        socket.on('state-update', handleResponse);
      });
    } catch (error) {
  ...
  async testUserExperience() {
    ...
    try {
      ...
      const manualRefreshTest = await this.testManualRefresh();
  ...
  async testManualRefresh() {
    try {
      ...
      return new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => {
          resolve({ working: refreshCount >= 2, error: null });
        }, 5000);
        
        socket.on('connect', () => {
          // Simulate manual refresh requests
          socket.emit('get_agent_list', {});
          setTimeout(() => socket.emit('getAgents', {}), 1000);
          setTimeout(() => socket.emit('listen-to-agents', {}), 2000);
        });
  ...
  // Run all tests
  async runAllTests() {
    log('Starting Comprehensive Frontend Integration Tests...');
...
async function runComprehensiveTests() {
  ...
  try {
    ...
    const fs = require('fs');
    fs.writeFileSync(
      'COMPREHENSIVE_FRONTEND_INTEGRATION_TEST_REPORT.json', 
    ...
    log('\nDetailed report saved to: COMPREHENSIVE_FRONTEND_INTEGRATION_TEST_REPORT.json');
...
module.exports = { FrontendIntegrationTester, runComprehensiveTests };