...
export class EpisodicMemory {
  private events: Map<string, ExtendedEpisodicEvent> = new Map();
  private timelines: Map<string, EpisodicTimeline> = new Map();
  private emotionalTags: Map<string, EmotionalTag> = new Map();
  ...
  async storeEvent(event: ExtendedEpisodicEvent): Promise<void> {
    ...
    const extendedEvent: ExtendedEpisodicEvent = {
      ...event,
   ...
   */
  async query(query: MemoryQuery): Promise<ExtendedEpisodicEvent[]> {
    const results: ExtendedEpisodicEvent[] = [];
    ...
    for (const event of Array.from(this.events.values())) {
      ...
      const forgottenImportance = this.forgettingCurve.calculateAdjustedImportance(
        event.importance, 
        timeSinceEvent
      );
   ...
   */
  async getRecentEvents(timeWindowMs: number): Promise<ExtendedEpisodicEvent[]> {
    const cutoff = Date.now() - timeWindowMs;
    const recent: ExtendedEpisodicEvent[] = [];
   ...
   */
  async applyDecay(deltaTimeMs: number): Promise<void> {
    const deltaTimeDays = deltaTimeMs / (1000 * 60 * 60 * 24);
    
    for (const event of Array.from(this.events.values())) {
      const timeSinceEvent = (Date.now() - event.timestamp) / (1000 * 60 * 60 * 24);
      const adjustedImportance = this.forgettingCurve.calculateAdjustedImportance(
        event.importance,
        timeSinceEvent
      );
   ...
   */
  async consolidateEvents(eventIds: string[]): Promise<void> {
  ...
  getEventsByEmotion(emotionType: 'positive' | 'negative' | 'high_arousal' | 'low_arousal'): ExtendedEpisodicEvent[] {
    ...
    for (const event of Array.from(this.events.values())) {
      if (!event.emotional) continue;
      
      switch (emotionType) {
        case 'positive':
          if (event.emotional.valence > 0.3) results.push(event);
          break;
        case 'negative':
          if (event.emotional.valence < -0.3) results.push(event);
          break;
        case 'high_arousal':
          if (event.emotional.arousal > 0.7) results.push(event);
          break;
        case 'low_arousal':
          if (event.emotional.arousal < 0.3) results.push(event);
          break;
      }
    }
   ...
   */
  getCausalChain(startEventId: string, maxDepth: number = 5): ExtendedEpisodicEvent[] {
    const chain: ExtendedEpisodicEvent[] = [];
    const visited = new Set<string>();
    
    const explore = (eventId: string, depth: number): void => {
   ...
   */
  private calculateImportance(event: ExtendedEpisodicEvent): number {
  ...
  private async updateTimelines(event: ExtendedEpisodicEvent): Promise<void> {
  ...
  private async processEmotionalTags(event: ExtendedEpisodicEvent): Promise<void> {
    if (!event.emotional) return;
  ...
  private async extractPatterns(event: ExtendedEpisodicEvent): Promise<ExtractedPattern[]> {
  ...
  private isCausallyRelated(event1: ExtendedEpisodicEvent, event2: ExtendedEpisodicEvent): boolean {
    ...
    const distance = Math.sqrt(
      Math.pow(event1.location.x - event2.location.x, 2) +
  ...
  private calculateRelevanceScore(event: ExtendedEpisodicEvent, query: MemoryQuery): number {
    const queryLower = query.query.toLowerCase();
    let score = 0;
    
    if (event.type?.toLowerCase().includes(queryLower)) score += 0.6;
    if (event.action?.toLowerCase().includes(queryLower)) score += 0.5;
    if (event.outcome?.toLowerCase().includes(queryLower)) score += 0.4;
  ...
  private async cleanupLeastImportantEvents(): Promise<void> {
    if (this.events.size <= this.maxEvents * 0.8) return;
  ...
  private async cleanupOldTimelines(): Promise<void> {
...
export class ForgettingCurve {
  private decayConstant: number = 0.693; // Half-life of 1 day for average importance
  private importanceFactor: number = 2.0; // Higher importance decays slower
