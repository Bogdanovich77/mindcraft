...
export interface PersonalityTraits {
  ...
  openness: number;        // Willingness to try new strategies
  conscientiousness: number; // Planning and organization tendency
  extraversion: number;    // Social interaction preference
  agreeableness: number;   // Cooperation vs competition
  neuroticism: number;     // Stress reactivity and emotional stability
  
  // Gaming-Specific Traits (0-1 scale)
  riskTolerance: number;   // Willingness to take dangerous actions
  creativity: number;      // Innovative vs conventional approaches
  patience: number;        // Willingness to wait for optimal timing
  competitiveness: number; // Drive to win vs collaborative approach
  curiosity: number;       // Exploration and discovery drive
}

export interface PersonalityProfile {
  traits: PersonalityTraits;
  confidence: number;      // Overall confidence in decisions (0-1)
  adaptability: number;    // How quickly personality adapts (0-1)
  consistency: number;     // How consistent behavior remains (0-1)
}

export class PersonalitySystem {
  private profile: PersonalityProfile;
  private baselineTraits: PersonalityTraits;
   ...
   */
  private createDefaultTraits(): PersonalityTraits {
    return {
      openness: 0.5,
  ...
  calculateStressLevel(context: any): number {
    ...
    if (context.danger) stressModifier += 0.3;
    if (context.unknown) stressModifier += 0.2;
    if (context.time_pressure) stressModifier += 0.2;
    if (context.social_pressure) stressModifier += 0.1 * (1 - this.profile.traits.extraversion);
...
export interface PersonalityExperience {
  success: boolean;
  intensity: number; // 0-1, how strong the experience was
  traitInfluences: {
    [key in keyof PersonalityTraits]?: number; // -1 to 1 influence direction
  };
  context: any;
}