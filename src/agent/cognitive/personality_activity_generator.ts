...
export interface ActivityGenerationConfig {
  ...
  contextWeight: number;         // How much current context influences generation
  personalityWeight: number;      // How much personality influences generation
  refreshInterval: number;         // milliseconds between generations
}
...
export class PersonalityActivityGenerator {
  private config: ActivityGenerationConfig;
  private lastGenerationTime: number = 0;
  private activityHistory: PersonalityActivitySuggestion[] = [];

  constructor(
    private agentId: string,
    config?: Partial<ActivityGenerationConfig>
  ) {
    this.config = {
      enabled: true,
      maxActivities: 3,
   ...
   */
  async generateActivities(agentState: AgentState): Promise<PersonalityActivitySuggestion[]> {
    ...
    try {
      ...
      const activities: PersonalityActivitySuggestion[] = [];
      ...
      const filteredActivities = this.filterAndPrioritizeActivities(
        activities, 
        profile, 
        personality, 
        context
   ...
   */
  private createActivityProfile(personality: PersonalityTraits): PersonalityActivityProfile {
    return {
   ...
   */
  private analyzeCurrentContext(agentState: AgentState): any {
    return {
  ...
  private generateExplorationActivities(
    profile: PersonalityActivityProfile,
    personality: PersonalityTraits,
    context: any
  ): PersonalityActivitySuggestion[] {
    const activities: PersonalityActivitySuggestion[] = [];
  ...
  private filterAndPrioritizeActivities(
    activities: PersonalityActivitySuggestion[],
    profile: PersonalityActivityProfile,
    personality: PersonalityTraits,
    context: any
  ): PersonalityActivitySuggestion[] {
    ...
    let filtered = activities.filter(activity => 
      activity.personalityAlignment >= this.config.minPersonalityAlignment
    );

    // Filter by context appropriateness
    filtered = filtered.filter(activity => 
      this.isContextAppropriate(activity, context)
    );
   ...
   */
  private isContextAppropriate(activity: PersonalityActivitySuggestion, context: any): boolean {
   ...
   */
  private ensureDiversity(activities: PersonalityActivitySuggestion[]): PersonalityActivitySuggestion[] {
    const categories = new Set<ActivityCategory>();
    const diverse: PersonalityActivitySuggestion[] = [];
  ...
  private calculateExplorationPreference(personality: PersonalityTraits): number {
    return (
  ...
  private calculateSocialPreference(personality: PersonalityTraits): number {
    return (
  ...
  private calculateBuildingPreference(personality: PersonalityTraits): number {
    return (
  ...
  private calculateCombatPreference(personality: PersonalityTraits): number {
    return (
      personality.combatAggression * 0.4 +
  ...
  private calculateCraftingPreference(personality: PersonalityTraits): number {
    return (
      personality.buildingCreativity * 0.3 +
      personality.conscientiousness * 0.3 +
      personality.socialTendency * 0.2 +
  ...
  private calculateActivityPriority(personalityAlignment: number, context: any): number {
    let priority = personalityAlignment * 10; // Base priority from alignment

    // Context modifiers
    if (context.dangerLevel === 'high') priority += 5;
    if (context.health < 30) priority += 3;
    if (context.food < 30) priority += 2;
    if (context.nearbyEntities > 5) priority -= 1; // Less priority when crowded

    return Math.max(1, Math.min(20, priority));
  }

  private calculateActivityConfidence(activityType: string, personality: PersonalityTraits, context: any): number {
    ...
    relevantTraits.forEach(trait => {
      if (trait in personality) {
        const traitValue = personality[trait as keyof PersonalityTraits] as number;
        confidence += traitValue * 0.1;
      }
    });

    // Context-based confidence
    if (context.dangerLevel === 'low') confidence += 0.1;
    if (context.health > 50) confidence += 0.1;
    if (context.inventoryLoad < 0.8) confidence += 0.1;

    return Math.max(0.1, Math.min(1.0, confidence));
  }

  private getRelevantTraits(activityType: string): string[] {
  ...
  private calculateInventoryLoad(inventory: any[]): number {
  ...
  private estimateTerrainType(agentState: AgentState): string {
    const position = agentState.context.position;
    
    // Simple terrain estimation based on y-coordinate
    if (position.y < 60) return 'underground';
    if (position.y < 70) return 'surface';
    if (position.y < 80) return 'hills';
    return 'mountains';
  }

  private assessDangerLevel(agentState: AgentState): string {
    const nearbyEntities = agentState.context.nearbyEntities || [];
    const hostileEntities = nearbyEntities.filter(entity => entity.hostile);
    
    if (hostileEntities.length > 3) return 'high';
    if (hostileEntities.length > 0) return 'medium';
    return 'low';
  }

  private analyzeSocialContext(agentState: AgentState): any {
    const nearbyEntities = agentState.context.nearbyEntities || [];
  ...
  private isUnderwaterExplorationSafe(context: any): boolean {
  ...
  private updateActivityHistory(activities: PersonalityActivitySuggestion[]): void {
