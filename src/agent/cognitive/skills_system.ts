...
import { SocialState, Relationship } from '../langgraph/interfaces.js';
...
export class SkillsSystem {
  ...
  private socialObservations: Map<string, any[]> = new Map(); // agentId -> observations
  private teachingHistory: Map<string, number> = new Map(); // skillType -> students taught
  ...
  learnFromObservation(
    ...
    const observationEvent: ExperienceEvent = {
  ...
  teachSkill(
    ...
    const teachingEvent: ExperienceEvent = {
  ...
  collaborativeSkillExecution(
    ...
    const collaborationEvent: ExperienceEvent = {
  ...
  getSocialSkillReputation(skillType: SkillType): {
    ...
    if (this.socialState) {
      ...
      const avgTrust = relationshipArray.reduce((sum: number, relationship: Relationship) => {
  ...
  identifySocialSkillGaps(): {
    skillType: SkillType;
    gap: number; // 0-1, higher means bigger gap
   ...
   */
  private generateSocialSkillRecommendation(skill: Skill, gap: number): string {
    const recommendations = [
  ...
  getSocialLearningInsights(): {
    ...
    const mostTaughtSkills = Array.from(this.teachingHistory.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(entry => entry[0] as SkillType);
    ...
    this.recentProgress.forEach(progress => {
      if (progress.timeSpent > 0) {
        ...
        const socialProgress = this.recentProgress.filter(p =>
          Math.abs(p.timeSpent - progress.timeSpent) < 1000
        ).length;
    ...
    const socialLearningRate = this.recentProgress.filter(progress =>
      progress.experienceGained > 10
    ).length / Math.max(1, this.recentProgress.length);
