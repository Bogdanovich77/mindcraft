...
export class PlanningEngine {
   ...
   */
  async createPlan(request: PlanningRequest, agentState: AgentState): Promise<PlanningResult> {
    ...
    try {
      ...
      let resourceAssessment: ResourceAssessmentResult | null = null;
      if (this.config.enableResourceAssessment) {
        resourceAssessment = await this.resourceAssessment.assessResources(
          request.resourceRequirements || { items: {}, tools: [], location: undefined, time: 0, assistance: [] },
          agentState
        );
      }
      ...
      let feasibility: FeasibilityAnalysisResult | null = null;
   ...
   */
  async updatePlan(planId: string, updates: Partial<Plan>, agentState: AgentState): Promise<PlanningResult> {
    const startTime = Date.now();
   ...
   * Execute a plan step
   */
  async executeStep(planId: string, stepId: string, agentState: AgentState): Promise<boolean> {
    try {
   ...
   * Complete a plan
   */
  async completePlan(planId: string, agentState: AgentState): Promise<boolean> {
    try {
      const plan = this.activePlans.get(planId);
   ...
   * Cancel a plan
   */
  async cancelPlan(planId: string, reason?: string): Promise<boolean> {
    try {
      const plan = this.activePlans.get(planId);
  ...
  private validatePlanningRequest(request: PlanningRequest, agentState: AgentState): { isValid: boolean; reason?: string } {
    if (!request.goalId) {
  ...
  private createPlanningContext(request: PlanningRequest, agentState: AgentState): PlanningContext {
    return {
  ...
  private async generateInitialPlan(request: PlanningRequest, context: PlanningContext, agentState: AgentState): Promise<Plan> {
  ...
  private determinePlanType(request: PlanningRequest, context: PlanningContext): PlanType {
  ...
  private async generatePlanSteps(request: PlanningRequest, context: PlanningContext, agentState: AgentState): Promise<PlanStep[]> {
  ...
  private async optimizePlan(plan: Plan, feasibility: FeasibilityAnalysisResult | null, resourceAssessment: ResourceAssessmentResult | null): Promise<Plan> {
  ...
  private async generateAlternatives(plan: Plan, feasibility: FeasibilityAnalysisResult | null, agentState: AgentState): Promise<AlternativePlan[]> {
  ...
  private convertFeasibilityResult(feasibility: FeasibilityAnalysisResult | null): FeasibilityResult | null {
  ...
  private createPlanningResult(plan: Plan | null, success: boolean, message: string, startTime: number): PlanningResult {
    return {
      plan,
      success,
      resourceAssessment: null,
  ...
  private addToHistory(result: PlanningResult): void {
