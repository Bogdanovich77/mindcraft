...
export class SkillsBridge {
  private config: SkillsBridgeConfig;
  ...
  // Legacy integration
  private legacySkillMappings: Map<string, SkillMapping> = new Map();
  private legacyCache: Map<string, LegacySkillData> = new Map();
  ...
  constructor(
    ...
    synergySystem: SkillSynergySystem,
    milestoneSystem: SkillMilestoneSystem,
    experienceTracker: ExperienceTracker,
    config?: Partial<SkillsBridgeConfig>
  ) {
   ...
   */
  private initializeStatistics(): BridgeStatistics {
    return {
   ...
   */
  private performAutoMigration(): void {
    try {
   ...
   */
  private extractLegacySkillsData(): LegacySkillData[] {
    const legacySkills: LegacySkillData[] = [];
   ...
   */
  private migrateLegacySkill(legacySkill: LegacySkillData): void {
    ...
    try {
      ...
      if (!existingSkill) {
        ...
        if (newSkill) {
          ...
          this.learningEngine.processExperience(
            newSkill,
            experienceEvent,
            experienceEvent.context
          );
        }
      } else {
   ...
   */
  private async syncToLegacy(): Promise<void> {
    ...
    for (const skill of allSkills) {
      try {
        ...
        if (!reverseMappingName) continue;
   ...
   */
  private async syncFromLegacy(): Promise<void> {
   ...
   */
  private findReverseMapping(skillType: SkillType): string | null {
   ...
   * Check if legacy data has changed
   */
  private hasLegacyDataChanged(cached: LegacySkillData, current: LegacySkillData): boolean {
    return (
   ...
   */
  private updateLegacySkillData(legacyName: string, data: LegacySkillData): void {
   ...
   */
  public executeLegacySkill(skillName: string, ...args: any[]): any {
    if (!this.config.enableLegacyCompatibility) {
    ...
    try {
      ...
      const legacyFunction = (LegacySkills as any)[skillName];
   ...
   */
  private trackLegacyExecution(skillName: string, args: any[], result: any): void {
   ...
   */
  private calculateLegacyExperience(skillName: string, args: any[], result: any): number {
    const mapping = this.legacySkillMappings.get(skillName);
    if (!mapping) return 1;
    ...
    if (result && typeof result === 'object') {
      if (result.success) baseExperience *= 1.5;
      if (result.quality) baseExperience *= result.quality;
      if (result.difficulty) baseExperience *= (1 + result.difficulty);
    }
   ...
   */
  private createLegacyContext(args: any[]): ExperienceContext {
    return {
   ...
   */
  private assessLegacyExecutionQuality(result: any): number {
    if (!result) return 0.5;
    if (result.quality !== undefined) return Math.max(0, Math.min(1, result.quality));
    if (result.success !== undefined) return result.success ? 0.8 : 0.3;
    return 0.6;
  }
   ...
   */
  private assessLegacyExecutionDifficulty(skillName: string, args: any[]): number {
   ...
   */
  private assessLegacyExecutionSuccess(result: any): boolean {
   ...
   * Get skill mappings
   */
  public getSkillMappings(): Map<string, SkillMapping> {
  ...
  public exportConfiguration(): {
    ...
    mappings: Array<{ legacyName: string; mapping: SkillMapping }>;
