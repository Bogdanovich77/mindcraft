...
export interface Motivation {
  type: MotivationType;
  strength: number;           // Current intensity (0-1)
  persistence: number;        // How long motivation lasts (0-1)
  satiation: number;          // Current satisfaction level (0-1)
  satiationThreshold: number; // Level at which motivation decreases (0-1)
  decayRate: number;          // Natural decay over time (0-1)
...
export interface MotivationProfile {
  ...
  motivationVolatility: number; // How quickly motivations change (0-1)
}

export interface MotivationEvent {
  type: 'success' | 'failure' | 'progress' | 'setback';
  motivationType: MotivationType;
  impact: number;             // Strength of impact (-1 to 1)
  context: any;
}
...
export class MotivationSystem {
  ...
  private readonly SATIATION_DECAY_RATE = 0.02;
   ...
   */
  private createDefaultMotivations(): Map<MotivationType, Motivation> {
    const motivations = new Map<MotivationType, Motivation>();
   ...
   */
  private processMotivationEvent(event: MotivationEvent): void {
   ...
   */
  private updatePrimaryMotivation(): void {
    let maxStrength = 0;
    let primaryType: MotivationType = 'survival';
   ...
   */
  getPrimaryMotivation(): { type: MotivationType; strength: number } {
   ...
   */
  private getActionMotivationWeight(actionType: string, motivationType: MotivationType): number {
    const weights: Record<string, Record<MotivationType, number>> = {
   ...
   */
  generateGoals(): Array<{ type: string; priority: number; motivation: MotivationType }> {
    const goals: Array<{ type: string; priority: number; motivation: MotivationType }> = [];
   ...
   */
  private getGoalTypesForMotivation(motivationType: MotivationType): string[] {
    const goalMap: Record<MotivationType, string[]> = {
  ...
  static fromLegacyProfile(legacyProfile: any): MotivationSystem {
    const motivationProfile: Partial<MotivationProfile> = {};
