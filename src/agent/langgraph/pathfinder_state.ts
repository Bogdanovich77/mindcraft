...
export interface PathfinderOperation {
  ...
  startTime: number;
  timeout: number;
  goal: any; // pf.goals.Goal
  status: 'active' | 'completed' | 'interrupted' | 'failed' | 'timeout';
  priority: number;
  context: {
    source: string; // Which function initiated this operation
...
export class InterruptiblePathfinder {
  private bot: Bot;
  private activeOperations: Map<string, PathfinderOperation> = new Map();
  private operationCounter: number = 0;
  private cleanupTimeouts: Map<string, NodeJS.Timeout> = new Map();
  private metrics: PathfinderMetrics = {
    totalOperations: 0,
  ...
  private readonly CLEANUP_TIMEOUT = 5000; // 5 seconds for cleanup
  ...
  async startOperation(
    ...
    const operation: PathfinderOperation = {
      id: operationId,
      type,
      startTime: Date.now(),
   ...
   */
  completeOperation(operationId: string, result: 'success' | 'interrupted' | 'failed' = 'success'): void {
   ...
   */
  stopAllOperations(reason: string = 'emergency_interrupt'): void {
   ...
   */
  private forceStopOperation(operationId: string, reason: string): void {
    const operation = this.activeOperations.get(operationId);
    if (!operation) return;
   ...
   */
  private performOperationCleanup(operationId: string): void {
    try {
  ...
  validateState(): PathfinderStateValidation {
    const issues: string[] = [];
    const corruptedOperations: PathfinderOperation[] = [];
   ...
   */
  private setupPathfinderListeners(): void {
   ...
   */
  private updateMetrics(type: 'start' | 'complete' | 'interrupt' | 'fail' | 'timeout', operation: PathfinderOperation): void {
    switch (type) {
      case 'start':
...
export class PathfinderStateManager {
  private static instance: PathfinderStateManager | null = null;
  private pathfinders: Map<string, InterruptiblePathfinder> = new Map();
  private globalMetrics: PathfinderMetrics = {
    totalOperations: 0,
    successfulOperations: 0,
    interruptedOperations: 0,
    failedOperations: 0,
   ...
   */
  stopAllOperations(reason: string = 'global_interrupt'): void {
  ...
  validateAllStates(): Record<string, PathfinderStateValidation> {
    const results: Record<string, PathfinderStateValidation> = {};
  ...
  getAllActiveOperations(): Record<string, PathfinderOperation[]> {
    const results: Record<string, PathfinderOperation[]> = {};
