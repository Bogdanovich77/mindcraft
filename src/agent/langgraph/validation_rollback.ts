...
import { AgentState, Goal } from './interfaces.js';
...
// Re-export EnhancedProfile since it's not exported from profile_adapter
...
export class ValidationRollbackManager {
    ...
    private maxRollbackPoints: number = 10;
     ...
     * Validate profile structure
     */
    private validateProfile(profile: EnhancedProfile): ValidationResult {
        const issues: ValidationIssue[] = [];
     ...
     */
    private validateAgentState(agentState: AgentState): ValidationResult {
        const issues: ValidationIssue[] = [];
     ...
     */
    private validateCompatibilityLayer(compatibilityLayer: CompatibilityLayer): ValidationResult {
        const issues: ValidationIssue[] = [];
     ...
     * Validate migration status
     */
    private validateMigrationStatus(profile: EnhancedProfile): ValidationResult {
        const issues: ValidationIssue[] = [];
     ...
     * Validate rollback data
     */
    private validateRollbackData(data: any): ValidationResult {
        const issues: ValidationIssue[] = [];
     ...
     */
    private calculateSeverity(issues: ValidationIssue[]): 'info' | 'warning' | 'error' | 'critical' {
     ...
     */
    private getOldestRollbackPoint(): string | null {
     ...
     * Check migration manager health
     */
    private checkMigrationManagerHealth(): ComponentHealth {
        const issues: string[] = [];
     ...
     * Check profile adapter health
     */
    private checkProfileAdapterHealth(): ComponentHealth {
        const issues: string[] = [];
        let status: 'healthy' | 'degraded' | 'unhealthy' | 'critical' = 'healthy';
     ...
     */
    private checkCompatibilityLayerHealth(compatibilityLayer: CompatibilityLayer): ComponentHealth {
        const issues: string[] = [];
        let status: 'healthy' | 'degraded' | 'unhealthy' | 'critical' = 'healthy';
     ...
     */
    private calculateHealthMetrics(components: ComponentHealth[]): HealthMetrics {
        const criticalCount = components.filter(c => c.status === 'critical').length;
        const unhealthyCount = components.filter(c => c.status === 'unhealthy').length;
