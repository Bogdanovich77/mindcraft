...
import { AgentState, PurposeState, PersonalityTraits } from './interfaces.js';
...
export class ProfileAdapter {
    ...
    private options: ProfileAdapterOptions;
    private profileCache: Map<string, EnhancedProfile> = new Map();
     ...
     */
    async loadProfile(profilePath: string): Promise<EnhancedProfile> {
        try {
            // Check cache first
     ...
     */
    async saveProfile(profilePath: string, profile: EnhancedProfile): Promise<void> {
        try {
            // Update metadata
     ...
     */
    async migrateProfile(profilePath: string): Promise<EnhancedProfile> {
        try {
    ...
    extractAgentState(profile: EnhancedProfile): AgentState | null {
        if (!profile.agentState) {
            ...
            if (profile.npc) {
                ...
                profile.agentState = partialState as AgentState;
            }
        }
     ...
     */
    private async loadLegacyProfile(profilePath: string): Promise<any> {
     ...
     */
    private async enhanceProfile(legacyProfile: any): Promise<EnhancedProfile> {
        const enhancedProfile: EnhancedProfile = {
            ...legacyProfile,
     ...
     */
    private async saveEnhancedProfile(profilePath: string, profile: EnhancedProfile): Promise<void> {
        try {
            const fs = await import('fs/promises');
     ...
     */
    private async createBackup(profilePath: string, profile: EnhancedProfile): Promise<void> {
        try {
            const fs = await import('fs/promises');
     ...
     * Validate profile
     */
    private validateProfile(profile: EnhancedProfile): ProfileValidation {
     ...
     */
    private syncLegacyData(profile: EnhancedProfile, agentState: AgentState): void {
        try {
...
export class ProfileUtils {
    ...
    static isHealthy(profile: EnhancedProfile): boolean {
        ...
        if (stats.compatibilityMode === 'new_only' && !stats.hasAgentState) return false;
        if (stats.age > 24 * 60 * 60 * 1000 && !stats.lastMigrated) return false; // Old profile not migrated
