...
import { AgentState, Goal, SkillState, MemoryState } from './interfaces.js';
...
export class MigrationManager {
     ...
     */
    private validateLegacyProfile(profileData: any): ValidationResult {
     ...
     * Validate migrated data
     */
    private validateMigratedData(agentState: Partial<AgentState>): ValidationResult {
        const result: ValidationResult = {
            isValid: true,
            errors: [],
            warnings: []
    ...
    private convertToAgentState(
        dataAdapter: LegacyNPCDataAdapter,
        memoryAdapter: LegacyMemoryAdapter,
     ...
     */
    private createRollbackSnapshot(profileData: any): any {
     ...
     */
    private preserveOriginalIds(agentState: Partial<AgentState>): void {
...
export class MigrationUtils {
    ...
    static estimateComplexity(profileData: any): 'simple' | 'moderate' | 'complex' {
        if (!profileData.npc) return 'simple';
        
        const goalCount = profileData.npc.goals?.length || 0;
        const memoryCount = Object.keys(profileData.memory_bank || {}).length;
        
        if (goalCount > 10 || memoryCount > 50) return 'complex';
        if (goalCount > 5 || memoryCount > 20) return 'moderate';
        return 'simple';
    }
