...
import {
  ...
  EmotionalResponse,
  MoodState,
  EmotionalIntelligence as EmotionalIntelligenceMetrics,
  Evidence,
  EvidenceType
} from './tom_types.js';
...
export interface EmotionalIntelligenceConfig {
  ...
  emotionDecayRate: number; // 0-1 per minute
  moodInfluenceFactor: number; // 0-1
  empathyThreshold: number; // 0-1
  contagionThreshold: number; // 0-1
...
export class EmotionalIntelligence {
  ...
  private emotionPatterns: Map<Emotion, EmotionCuePattern>;
  private emotionalHistory: Map<string, EmotionalState[]>;
  private empathyHistory: Map<string, Map<string, number>>; // observer -> target -> empathy level
  private moodStates: Map<string, MoodState>;
   ...
   */
  private initializeMetrics(): EmotionalIntelligenceMetrics {
    return {
      recognition: 0.5,
      understanding: 0.5,
      regulation: 0.5,
   ...
   */
  public recognizeEmotions(context: EmotionRecognitionContext): Map<Emotion, number> {
    const recognizedEmotions = new Map<Emotion, number>();
  ...
  private calculateEmotionScore(
    pattern: EmotionCuePattern,
    context: EmotionRecognitionContext
  ): number {
    let cueScore = 0;
    let contextScore = 0;
    ...
    const matchingCues = context.behavioralCues.filter(cue => 
      pattern.cues.some(patternCue => 
        cue.type === patternCue.type && cue.intensity >= patternCue.threshold
      )
    );
    ...
    const matchingContextFactors = contextFactors.filter(factor =>
      pattern.contextFactors.includes(factor)
    );
    ...
    if (pattern.personalityCorrelation && context.personality) {
      personalityScore = this.calculatePersonalityCorrelation(
        pattern.personalityCorrelation,
        context.personality
      );
    }
   ...
   */
  private extractContextFactors(context: EmotionRecognitionContext): string[] {
    const factors: string[] = [];
    
    // Extract from environment
    if (context.environment) {
      if (context.environment.danger_level > 0.7) factors.push('danger');
      if (context.environment.danger_level > 0.5) factors.push('threat');
      if (context.environment.resources_available) factors.push('resource_gain');
      if (context.environment.obstacles) factors.push('obstacle');
    }

    // Extract from social context
    if (context.socialContext) {
      if (context.socialContext.allies) factors.push('cooperation');
      if (context.socialContext.conflicts) factors.push('conflict');
      if (context.socialContext.social_positive) factors.push('social_positive');
      if (context.socialContext.social_rejection) factors.push('social_rejection');
    }

    // Extract from recent actions
    for (const action of context.recentActions.slice(-5)) {
      if (action.outcome === 'success') factors.push('success');
      if (action.outcome === 'failure') factors.push('failure');
      if (action.type === 'attack') factors.push('aggression');
      if (action.type === 'flee') factors.push('escape');
    }
   ...
   */
  private applyMoodInfluence(emotions: Map<Emotion, number>, agentId: string): void {
  ...
  private applyPersonalityInfluence(
    ...
    const personalityEmotions: Array<{
   ...
   */
  private getMoodState(agentId: string): MoodState {
   ...
   */
  public updateMoodState(agentId: string, emotions: Map<Emotion, number>): void {
    if (!this.config.enableMoodModeling) return;
  ...
  private calculateEmotionalConsistency(
    current: Map<Emotion, number>,
    previous: Map<Emotion, number>
  ): number {
    if (current.size === 0 && previous.size === 0) return 1.0;
    if (current.size === 0 || previous.size === 0) return 0.0;
   ...
   */
  public simulateEmpathy(context: EmpathySimulationContext): number {
    try {
   ...
   */
  private calculateSituationEmpathy(situation: string): number {
    const empathySituations: Record<string, number> = {
      'danger': 0.8,
      'pain': 0.9,
      'loss': 0.7,
      'failure': 0.6,
   ...
   * Store empathy history
   */
  private storeEmpathyHistory(observerId: string, targetId: string, empathyScore: number): void {
   ...
   */
  public predictEmotionalResponse(
    agentId: string,
    situation: string,
    context: any
    ...
    try {
      ...
      if (emotionalHistory.length > 0) {
        ...
        const similarPastResponses = pastResponses.filter(state =>
          state.currentEmotions.has(predictedEmotion)
        );

        if (similarPastResponses.length > 0) {
          const avgPastIntensity = similarPastResponses.reduce((sum, state) =>
            sum + (state.currentEmotions.get(predictedEmotion) || 0), 0
          ) / similarPastResponses.length;
   ...
   */
  public simulateEmotionalContagion(
    sourceId: string,
    targetId: string,
    sourceEmotions: Map<Emotion, number>,
    context: any
  ): Map<Emotion, number> {
    const contagiousEmotions = new Map<Emotion, number>();
   ...
   */
  private updateRecognitionMetrics(recognizedEmotions: Map<Emotion, number>): void {
    if (recognizedEmotions.size === 0) return;
   ...
   */
  private updateEmpathyMetrics(empathyScore: number): void {
   ...
   */
  public getEmotionalHistory(agentId: string): EmotionalState[] {
   ...
   */
  public getPublicMoodState(agentId: string): MoodState {
   ...
   */
  public getEmpathyLevel(observerId: string, targetId: string): number {
   ...
   * Get configuration
   */
  public getConfig(): EmotionalIntelligenceConfig {
    return { ...this.config };
  }
   ...
   */
  public updateConfig(newConfig: Partial<EmotionalIntelligenceConfig>): void {
...
export class EmotionalIntelligenceFactory {
  ...
  static createDefault(): EmotionalIntelligence {
    const config: EmotionalIntelligenceConfig = {
