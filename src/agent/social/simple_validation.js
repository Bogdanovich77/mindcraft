...
try {
  ...
  const prediction = await tomEngine.predictBehavior(
    'test-agent-1',
    testContext,
    5000
  );
  ...
  const perspective = await tomEngine.takePerspective(
    'test-agent-1',
    testContext,
    2000
  );
  ...
  const deception = await tomEngine.detectDeception(
    'test-agent-1',
    { message: 'I will help you', behavioralCues: ['nervous'] },
    testContext
  );
