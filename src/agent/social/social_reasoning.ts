...
import { AgentRelationship, RelationshipNetwork } from './relationship_types.js';
...
export interface SocialReasoningConfig {
  ...
  contextRetentionPeriod: number; // milliseconds
}
...
export class SocialReasoningEngine {
  private config: SocialReasoningConfig;
  private situationPatterns: Map<SituationType, SocialSituationPattern>;
  private culturalContexts: Map<string, CulturalContext>;
  private socialNorms: Map<string, SocialNorm[]>;
  private reasoningHistory: Map<string, SocialReasoningOutput[]>;
   ...
   */
  private initializeSituationPatterns(): void {
   ...
   */
  private initializeSocialNorms(): void {
   ...
   */
  public analyzeSocialContext(input: SocialReasoningInput): SocialReasoningOutput {
    ...
    try {
      ...
      const output: SocialReasoningOutput = {
        situationAssessment,
        groupDynamics,
        socialNorms,
        culturalContext,
   ...
   */
  private assessSocialSituation(input: SocialReasoningInput): SocialSituation {
    const situationScores = new Map<SituationType, number>();
  ...
  private calculateSituationScore(
    pattern: SocialSituationPattern,
    input: SocialReasoningInput
  ): number {
    ...
    const matchingIndicators = pattern.indicators.filter(indicator =>
      this.hasIndicator(input, indicator)
    );
    
    if (pattern.indicators.length > 0) {
      indicatorScore = matchingIndicators.reduce((sum, indicator) => 
        sum + indicator.weight, 0) / 
        pattern.indicators.reduce((sum, indicator) => sum + indicator.weight, 0);
    }
    
    // Calculate context score
    const contextString = JSON.stringify(input).toLowerCase();
    const matchingContext = pattern.contextRequirements.filter(requirement =>
      contextString.includes(requirement.toLowerCase())
    );
   ...
   */
  private hasIndicator(input: SocialReasoningInput, indicator: any): boolean {
    const contextString = JSON.stringify(input).toLowerCase();
  ...
  private analyzePowerDynamics(
    ...
    const sources = new Map<string, PowerSource[]>();
    ...
    for (const participant of input.nearbyAgents) {
      ...
      const agentSources: PowerSource[] = [];
    ...
    const connections = new Map<string, InfluenceConnection>();
      ...
      for (const targetId of input.nearbyAgents) {
        if (sourceId !== targetId) {
          ...
          if (sourceRelationship && sourceRelationship.trust.level > 0.5) {
            const connection: InfluenceConnection = {
              from: sourceId,
              to: targetId,
    ...
    const influenceNetwork: InfluenceNetwork = {
      connections,
      centrality,
   ...
   */
  private analyzeCommunicationPattern(input: SocialReasoningInput): CommunicationPattern {
    // Simple communication analysis based on context
    let flow: CommunicationFlow = CommunicationFlow.NETWORK;
    let styles = new Map<string, CommunicationStyle>();
    ...
    let nonVerbalCues: NonVerbalCue[] = [];
  ...
  private analyzeGroupDynamics(
    ...
    let leadership: LeadershipStructure;
    let subgroups: Subgroup[] = [];
    let conflict: ConflictLevel = ConflictLevel.LOW;
  ...
  private identifySocialNorms(
    ...
    const applicableNorms: SocialNorm[] = [];
    ...
    return applicableNorms.filter(norm => 
      norm.context === 'all_interactions' || 
   ...
   */
  private isContextRelevant(context: string, input: SocialReasoningInput): boolean {
   ...
   * Determine cultural context
   */
  private determineCulturalContext(input: SocialReasoningInput): CulturalContext {
   ...
   */
  private createNormComplianceRecommendation(socialNorms: SocialNorm[]): SocialRecommendation | null {
   ...
   */
  private storeReasoningHistory(agentId: string, output: SocialReasoningOutput): void {
    ...
    const filteredHistory = history.filter(output => 
      Date.now() - output.processingTime < cutoffTime
    );
   ...
   */
  private createDefaultSituation(): SocialSituation {
    return {
   ...
   */
  private createDefaultGroupDynamics(): GroupDynamics {
    return {
      cohesion: 0.5,
      leadership: {
   ...
   */
  private createDefaultCulturalContext(): CulturalContext {
   ...
   */
  public getReasoningHistory(agentId: string): SocialReasoningOutput[] {
   ...
   * Get configuration
   */
  public getConfig(): SocialReasoningConfig {
    return { ...this.config };
  }
   ...
   */
  public updateConfig(newConfig: Partial<SocialReasoningConfig>): void {
...
export class SocialReasoningEngineFactory {
  ...
  static createDefault(): SocialReasoningEngine {
    const config: SocialReasoningConfig = {
