...
import { MentalState, TheoryOfMindConfig, TheoryOfMindMetrics, SocialReasoningContext } from './tom_types.js';
...
export class TheoryOfMindEngine {
   ...
   */
  async initialize(personality: PersonalityTraits): Promise<void> {
    try {
  ...
  async processObservation(
    ...
    try {
      ...
      this.mentalModelManager.updateFromObservation(
        targetAgentId,
        observation,
        context,
        observerPersonality
      ...
      const socialContext: SocialReasoningContext = {
      ...
      const integratedMentalState = await this.integrateMentalState(
        mentalState,
  ...
  async predictBehavior(
    ...
    try {
      ...
      const emotionalPrediction = this.emotionalIntelligence.predictEmotionalResponse(
        agentId,
  ...
  async takePerspective(
    ...
    try {
      ...
      const emotionalPerspective = this.emotionalIntelligence.predictEmotionalResponse(
        agentId,
  ...
  async detectDeception(
    ...
    try {
      ...
      return {
        ...
        socialValidation,
        overallDeceptionProbability: this.calculateDeceptionProbability(
          deceptionAnalysis,
          emotionalInconsistencies,
          socialValidation
        ),
   ...
   */
  private createDefaultConfig(override?: Partial<TheoryOfMindConfig>): TheoryOfMindConfig {
   ...
   */
  private async integrateMentalState(
    baseMentalState: MentalState,
    intentionPredictions: any,
    emotionalAnalysis: any,
    socialContext: SocialReasoningContext
  ): Promise<MentalState> {
    // Create integrated mental state
    const integratedState: MentalState = {
      ...baseMentalState,
      intentions: {
  ...
  private calculateDeceptionProbability(
    deceptionAnalysis: any,
    emotionalInconsistencies: any,
...
export class TheoryOfMindFactory {
  ...
  static createPerformanceOptimized(): TheoryOfMindEngine {
    const config: Partial<TheoryOfMindConfig> = {
      maxMentalModels: 20,
