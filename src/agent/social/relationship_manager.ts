...
import { PersonalityTraits as CognitivePersonalityTraits } from '../cognitive/personality.js';
import { PersonalityTraits as LangGraphPersonalityTraits } from '../langgraph/interfaces.js';
...
export class RelationshipManager {
  ...
  private networkManager: SocialNetworkManager;
  private personalityCache: Map<string, CognitivePersonalityTraits>;
  ...
  processRelationshipUpdate(request: RelationshipUpdateRequest): void {
    ...
    relationship.trust = this.trustCalculator.updateTrust(
      relationship.trust,
      interaction,
      agentPersonality,
      targetReputation.globalScore
    );
    
    // Update friendship metrics
    relationship.friendship = this.updateFriendshipMetrics(
    ...
    relationship.respect = this.updateRespectMetrics(
      relationship.respect,
      interaction
    );
    
    // Update rivalry metrics
    relationship.rivalry = this.updateRivalryMetrics(
      relationship.rivalry,
      interaction
    );
    
    // Update collaboration metrics
    relationship.collaboration = this.updateCollaborationMetrics(
      relationship.collaboration,
      interaction
    );
    
    // Update communication metrics
    relationship.communication = this.updateCommunicationMetrics(
  ...
  searchRelationships(query: RelationshipQuery): RelationshipSearchResult {
    ...
    if (query.relationshipTypes) {
      relationships = relationships.filter(r => 
        r.history.some(interaction => query.relationshipTypes!.includes(interaction.type))
      );
    }
    
    if (query.timeRange) {
      relationships = relationships.filter(r => 
        r.history.some(interaction => 
          interaction.timestamp >= query.timeRange!.start && 
  ...
  getRelationshipSummary(): {
    ...
    topRelationships: Array<{ agentId: string; score: number; status: RelationshipStatus }>;
  } {
    const relationships = this.getAllRelationships();
    
    const totalRelationships = relationships.length;
    const activeRelationships = relationships.filter(r => 
      r.status !== RelationshipStatus.UNKNOWN
    ).length;
    ...
    // Status distribution
    const statusDistribution: Record<RelationshipStatus, number> = {} as any;
   ...
   */
  predictRelationshipEvolution(targetAgentId: string, timeframe: number = 7 * 24 * 60 * 60 * 1000): {
    ...
    const predictedTrust = this.trustCalculator.predictTrustLevel(
      relationship.trust,
      relationship.history.slice(-10), // Last 10 interactions
      timeframe
    );
    
    // Predict friendship based on recent trends
    const recentFriendshipTrend = this.calculateFriendshipTrend(relationship);
    const predictedFriendship = Math.max(0, Math.min(1, 
      relationship.friendship.level + (recentFriendshipTrend * timeframe / (7 * 24 * 60 * 60 * 1000))
    ));
  ...
  applyTimeDecay(): void {
    ...
    if (deltaTime < this.config.relationshipUpdateInterval) return;
  ...
  cleanupOldData(): void {
    ...
    this.relationshipNetwork.relationships.forEach(relationship => {
      ...
      relationship.history = relationship.history.filter(interaction => 
        interaction.timestamp > cutoffTime
      );
      
      // Remove old trust updates
      relationship.trust.updateHistory = relationship.trust.updateHistory.filter(update => 
    ...
    this.relationshipNetwork.relationshipHistory = this.relationshipNetwork.relationshipHistory.filter(event => 
   ...
   */
  private createEmptyNetwork(): RelationshipNetwork {
    return {
      agentId: this.agentId,
  ...
  private createNewRelationship(
    ...
    targetPersonality?: LangGraphPersonalityTraits
  ): AgentRelationship {
    const agentPersonality = this.convertToCognitivePersonality(this.getAgentPersonalityLangGraph(this.agentId));
    ...
    const friendship: FriendshipMetrics = {
   ...
   */
  private convertToCognitivePersonality(personality?: LangGraphPersonalityTraits): CognitivePersonalityTraits | undefined {
   ...
   */
  private getAgentPersonalityLangGraph(agentId: string): LangGraphPersonalityTraits {
    // Default personality - in real implementation, this would come from agent profile
    const defaultPersonality: LangGraphPersonalityTraits = {
      openness: 0.5,
   ...
   */
  private getAgentPersonality(agentId: string): CognitivePersonalityTraits {
  ...
  private updateFriendshipMetrics(
    current: FriendshipMetrics,
    interaction: RelationshipInteraction,
    agentPersonality: CognitivePersonalityTraits,
    targetPersonality: CognitivePersonalityTraits
  ): FriendshipMetrics {
   ...
   */
  private calculateRelationshipStatus(relationship: AgentRelationship): RelationshipStatus {
   ...
   */
  private calculateRelationshipTrends(relationship: AgentRelationship): RelationshipTrends {
   ...
   */
  private createEmptyTrends(): RelationshipTrends {
    return {
      trustTrend: 'stable' as any,
      friendshipTrend: 'stable' as any,
      respectTrend: 'stable' as any,
      collaborationTrend: 'stable' as any,
      overallTrend: 'stable' as any,
      prediction: {
   ...
   */
  private mapTrendToDirection(trend: number): any {
   ...
   */
  private limitHistorySize(relationship: AgentRelationship): void {
   ...
   */
  private updateSocialNetwork(relationship: AgentRelationship): void {
    ...
    this.networkManager.updateEdge(
      this.agentId,
      relationship.targetAgentId,
      weight,
      this.mapStatusToEdgeType(relationship.status),
      weight
    );
  }
   ...
   */
  private mapStatusToEdgeType(status: RelationshipStatus): any {
    switch (status) {
   ...
   */
  private extractSkillDemonstration(interaction: RelationshipInteraction): number {
   ...
   */
  private calculateFriendshipTrend(relationship: AgentRelationship): number {
   ...
   */
  private identifyRiskFactors(relationship: AgentRelationship): string[] {
   ...
   */
  private calculatePersonalNetworkMetrics(): any {
    const relationships = this.getAllRelationships();
   ...
   */
  private calculatePersonalClustering(): number {
   ...
   */
  private areAgentsConnected(agent1Id: string, agent2Id: string): boolean {
    const relationship1 = this.getRelationship(agent1Id);
    if (relationship1 && relationship1.targetAgentId === agent2Id) return true;
    
    const relationship2 = this.getRelationship(agent2Id);
    if (relationship2 && relationship2.targetAgentId === agent1Id) return true;
   ...
   */
  private calculatePersonalBetweenness(): number {
   ...
   */
  private calculateInfluencePropagation(): any {
   ...
   * Apply friendship decay
   */
  private applyFriendshipDecay(current: FriendshipMetrics, deltaTime: number): FriendshipMetrics {
