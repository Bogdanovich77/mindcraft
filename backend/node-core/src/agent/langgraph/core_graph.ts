...
// Type alias for the simplified StateGraph
type SimplifiedStateGraph = StateGraph<typeof AgentStateAnnotation>;
...
export class SimplifiedAgentGraph {
  private graph: SimplifiedStateGraph;
  private bot: Bot;
  private compiledGraph: CompiledStateGraph<typeof AgentStateAnnotation, any, any, any, any, any, any, any, any> | null = null;
  ...
  private setupGraph(): void {
    try {
      ...
      */
      ...
      this.graph.addEdge("execution" as any, END);
  ...
  async process(state: AgentState): Promise<AgentState> {
    ...
    try {
      const result = await this.compiledGraph.invoke((state as any).worldContext as any);
   ...
   */
  getStatus(): { isCompiled: boolean; nodeCount: number } {
