...
class OllamaModelTester {
    constructor(modelConfig = {}) {
        this.modelConfig = {
            model: modelConfig.model || 'Sweaterdog/Andy-4:micro-q8_0',
            url: modelConfig.url || 'http://127.0.0.1:11434',
    ...
    async testOllamaConnection() {
        try {
            ...
            if (testResponse.includes("Connection successful")) {
                ...
                console.log("  ⚠️ Ollama responded but unexpected output:", testResponse);
                return false;
            }
        } catch (error) {
...
- Location: Forest biome near a village`;
    ...
    async sendTestPrompt(messages) {
        ...
        try {
            ...
            const response = await this.sendOllamaRequest(userMessages, systemMessage);
    ...
    async sendOllamaRequest(turns, systemMessage) {
        ...
        try {
            const response = await fetch(`${this.modelConfig.url}/api/chat`, {
                method: 'POST',
                headers: {
...
async function main() {
    ...
    if (args.includes('--help') || args.includes('-h')) {
        console.log("Usage: node test_ollama_model.cjs [options]");
        console.log("Options:");
...
module.exports = { OllamaModelTester };