...
const { strictFormat } = require('./src/utils/text.js');

/**
 * Comprehensive Cerebras Model Test Suite for Mindcraft
 * Tests multi-turn agentic behavior with diverse personalities and intellectual awareness
 */

class CerebrasModelTester {
    constructor(modelConfig = {}) {
        this.modelConfig = {
            model: modelConfig.model || 'glm4.6',
            apiKey: modelConfig.apiKey || process.env.CEREBRAS_API_KEY,
            url: modelConfig.url || 'https://api.cerebras.ai/v1',
    ...
    async runAllTests() {
        console.log("🧠 Starting Comprehensive Cerebras Model Test Suite\n");
        ...
        try {
            ...
            await this.testCerebrasConnection();
    ...
    async testCerebrasConnection() {
        try {
            console.log(`  Connecting to ${this.modelConfig.url}...`);
            console.log(`  Testing model: ${this.modelConfig.model}`);
            
            if (!this.modelConfig.apiKey) {
                throw new Error("Cerebras API key not provided. Set CEREBRAS_API_KEY environment variable or pass in config.");
            }
            ...
            if (testResponse.includes("Connection successful")) {
                console.log("  ✅ Cerebras connection established successfully");
                return true;
            } else {
                console.log("  ⚠️ Cerebras responded but unexpected output:", testResponse);
                return false;
            }
        } catch (error) {
            console.log(`  ❌ Cerebras connection failed: ${error.message}`);
            return false;
        }
    }
...
- Location: Forest biome near a village`;
    ...
    async sendTestPrompt(messages) {
        ...
        try {
            ...
            const response = await this.sendCerebrasRequest(userMessages, systemMessage);
    ...
    async sendCerebrasRequest(turns, systemMessage) {
...
Respond naturally and intelligently, demonstrating your understanding of game mechanics and your ability to think strategically.`;
    ...
    async generateReport() {
        ...
        const filename = `cerebras_model_test_report_${timestamp}.md`;
    ...
    generateDetailedReport() {
        ...
        return `# Cerebras Model Test Report
...
// Main execution
async function main() {
    console.log("🧠 Cerebras Model Test Suite for Mindcraft");
    console.log("==========================================\n");
    ...
    if (args.includes('--help') || args.includes('-h')) {
        console.log("Usage: node test_cerebras_model.cjs [options]");
        console.log("Options:");
        console.log("  --model <name>    Model name (default: glm4.6)");
        console.log("  --api-key <key>  Cerebras API key (default: CEREBRAS_API_KEY env var)");
        console.log("  --url <url>       Cerebras API URL (default: https://api.cerebras.ai/v1)");
    ...
    if (apiKeyIndex !== -1 && args[apiKeyIndex + 1]) {
        modelConfig.apiKey = args[apiKeyIndex + 1];
    }
    ...
    console.log(`Testing model: ${modelConfig.model || 'glm4.6'}`);
    console.log(`Cerebras URL: ${modelConfig.url || 'https://api.cerebras.ai/v1'}`);
    console.log(`API Key: ${modelConfig.apiKey ? 'Provided' : 'From environment variable'}\n`);
    
    try {
        const tester = new CerebrasModelTester(modelConfig);
...
module.exports = { CerebrasModelTester };